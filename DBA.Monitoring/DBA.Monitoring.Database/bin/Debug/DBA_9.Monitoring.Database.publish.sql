...
BEGIN TRANSACTION
GO
PRINT N'Creating Table [dbo].[DeadlockLog]...';


GO
CREATE TABLE [dbo].[DeadlockLog] (
    [DeadlockLogID] BIGINT        IDENTITY (1, 1) NOT NULL,
    [CaptureTime]   DATETIME2 (2) NOT NULL,
    [DeadlockGraph] XML           NOT NULL,
    CONSTRAINT [PK_DeadlockLog] PRIMARY KEY NONCLUSTERED ([DeadlockLogID] ASC)
);
...
GO
PRINT N'Creating Index [dbo].[DeadlockLog].[IX_DeadlockLog_CaptureTime_DeadlockLogID]...';


GO
CREATE CLUSTERED INDEX [IX_DeadlockLog_CaptureTime_DeadlockLogID]
    ON [dbo].[DeadlockLog]([CaptureTime] ASC, [DeadlockLogID] ASC);
...
GO
PRINT N'Creating Default Constraint [dbo].[DF_DeadlockLog_CaptureTime]...';


GO
ALTER TABLE [dbo].[DeadlockLog]
    ADD CONSTRAINT [DF_DeadlockLog_CaptureTime] DEFAULT (sysdatetime()) FOR [CaptureTime];
...
GO
PRINT N'Creating View [dbo].[vRecentDeadlocks]...';
...
select
	dl.[DeadlockLogID],
	dl.[CaptureTime],
	[VictimProcessID] = dl.[DeadlockGraph].value(
		'(/EVENT_INSTANCE/TextData/deadlock/victim-list/victimProcess/@id)[1]',
		'nvarchar(100)'
	),
	[ProcessCount] = dl.[DeadlockGraph].value(
		'count(/EVENT_INSTANCE/TextData/deadlock/process-list/process)',
		'int'
	),
	[ResourceCount] = dl.[DeadlockGraph].value(
		'count(/EVENT_INSTANCE/TextData/deadlock/resource-list/*)',
		'int'
	),
	dl.[DeadlockGraph]
...
GO
PRINT N'Creating Procedure [dbo].[ProcessDeadlockMessages]...';
...
GO
PRINT N'Creating Queue [dbo].[DeadlockNotificationQueue]...';


GO
CREATE QUEUE [dbo].[DeadlockNotificationQueue]
    WITH ACTIVATION (STATUS = ON, PROCEDURE_NAME = [dbo].[ProcessDeadlockMessages], MAX_QUEUE_READERS = 1, EXECUTE AS SELF);
...
GO
PRINT N'Creating Service [DeadlockNotificationService]...';
